...
const EnterpriseDashboardLogisticienDisplay: React.FC = () => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('enterpriseDashboardConfig_logisticien');
    let parsed: any = null;
    if (saved) {
      try {
        parsed = JSON.parse(saved);
      } catch {
        localStorage.removeItem('enterpriseDashboardConfig_logisticien');
      }
    }
    ...
    if (shouldUpdate) {
      ...
      localStorage.setItem('enterpriseDashboardConfig_logisticien', JSON.stringify(parsed));
    }
  ...
  const onLayoutChange = (newLayout: any[], allLayouts?: any) => {
    ...
    if (config) {
      ...
      localStorage.setItem('enterpriseDashboardConfig_logisticien', JSON.stringify(updatedConfig));
    }
  };
  ...
  const handleToggleSize = (widgetId: string) => {
    setLayout(prevLayout => {
      ...
      setConfig(prevConfig => {
        ...
        localStorage.setItem('enterpriseDashboardConfig_logisticien', JSON.stringify(newConfig));
        return newConfig;
      });
  ...
  const handleRemoveWidget = (widgetId: string) => {
    ...
    if (currentLayoutItem) {
      ...
      const existingBackup = localStorage.getItem('enterpriseDashboardConfig_logisticien_backup');
      let backupConfig = { layout: { lg: [] } };
      if (existingBackup) {
        try {
          backupConfig = JSON.parse(existingBackup);
        } catch {
          localStorage.removeItem('enterpriseDashboardConfig_logisticien_backup');
        }
      }
      ...
      localStorage.setItem('enterpriseDashboardConfig_logisticien_backup', JSON.stringify(backupConfig));
  ...
  const handleAddWidget = (widgetId: string) => {
    ...
    const widgetToAdd = LogisticienWidgets.widgets.find(w => w.id === widgetId);
    if (!widgetToAdd) {
    ...
    const savedLayout = localStorage.getItem('enterpriseDashboardConfig_logisticien_backup');
  ...
  const handleRestoreAllWidgets = () => {
    ...
    const allWidgets = LogisticienWidgets.widgets;
  ...
  const handleSaveDashboard = () => {
    ...
    localStorage.setItem('enterpriseDashboardConfig_logisticien', JSON.stringify(currentConfig));
        ...
        {showAddWidgetModal && (
          <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md">
              <h2 className="text-lg font-bold mb-4">Ajouter un widget logisticien</h2>
              <div className="mb-2 text-xs text-gray-500">
                Widgets disponibles: {LogisticienWidgets.widgets.length} | 
                Widgets filtrés: {LogisticienWidgets.widgets.filter(widget => WIDGETS_LOGISTICIEN_IDS.includes(widget.id)).length}
              </div>
              <ul>
                {LogisticienWidgets.widgets
                  .filter(widget => WIDGETS_LOGISTICIEN_IDS.includes(widget.id))
                  .map(widget => {
