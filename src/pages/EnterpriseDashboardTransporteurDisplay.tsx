...
const EnterpriseDashboardTransporteurDisplay: React.FC = () => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('enterpriseDashboardConfig_transporteur');
    let parsed: any = null;
    if (saved) {
      try {
        parsed = JSON.parse(saved);
      } catch {
        localStorage.removeItem('enterpriseDashboardConfig_transporteur');
      }
    }
    ...
    if (shouldUpdate) {
      ...
      localStorage.setItem('enterpriseDashboardConfig_transporteur', JSON.stringify(parsed));
    }
  ...
  const onLayoutChange = (newLayout: any[], allLayouts?: any) => {
    ...
    if (config) {
      ...
      localStorage.setItem('enterpriseDashboardConfig_transporteur', JSON.stringify(updatedConfig));
    }
  };
  ...
  const handleToggleSize = (widgetId: string) => {
    setLayout(prevLayout => {
      ...
      setConfig(prevConfig => {
        ...
        localStorage.setItem('enterpriseDashboardConfig_transporteur', JSON.stringify(newConfig));
        return newConfig;
      });
  ...
  const handleRemoveWidget = (widgetId: string) => {
    ...
    if (currentLayoutItem) {
      ...
      const existingBackup = localStorage.getItem('enterpriseDashboardConfig_transporteur_backup');
      let backupConfig = { layout: { lg: [] } };
      if (existingBackup) {
        try {
          backupConfig = JSON.parse(existingBackup);
        } catch {
          localStorage.removeItem('enterpriseDashboardConfig_transporteur_backup');
        }
      }
      ...
      localStorage.setItem('enterpriseDashboardConfig_transporteur_backup', JSON.stringify(backupConfig));
  ...
  const handleAddWidget = (widgetId: string) => {
    ...
    const widgetToAdd = TransporteurWidgets.widgets.find(w => w.id === widgetId);
    if (!widgetToAdd) {
    ...
    const savedLayout = localStorage.getItem('enterpriseDashboardConfig_transporteur_backup');
  ...
  const handleRestoreAllWidgets = () => {
    ...
    const allWidgets = TransporteurWidgets.widgets;
  ...
  const handleSaveDashboard = () => {
    ...
    localStorage.setItem('enterpriseDashboardConfig_transporteur', JSON.stringify(currentConfig));
        ...
        {showAddWidgetModal && (
          <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md">
              <h2 className="text-lg font-bold mb-4">Ajouter un widget transporteur</h2>
              <div className="mb-2 text-xs text-gray-500">
                Widgets disponibles: {TransporteurWidgets.widgets.length} | 
                Widgets filtrés: {TransporteurWidgets.widgets.filter(widget => WIDGETS_TRANSPORTEUR_IDS.includes(widget.id)).length}
              </div>
              <ul>
                {TransporteurWidgets.widgets
                  .filter(widget => WIDGETS_TRANSPORTEUR_IDS.includes(widget.id))
                  .map(widget => {
