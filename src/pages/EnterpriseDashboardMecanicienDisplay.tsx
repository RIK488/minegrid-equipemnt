...
const EnterpriseDashboardMecanicienDisplay: React.FC = () => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('enterpriseDashboardConfig_mecanicien');
    ...
    if (shouldUpdate) {
      ...
      localStorage.setItem('enterpriseDashboardConfig_mecanicien', JSON.stringify(parsed));
    }
  ...
  const onLayoutChange = (newLayout: any[], allLayouts?: any) => {
    ...
    if (config) {
      ...
      localStorage.setItem('enterpriseDashboardConfig_mecanicien', JSON.stringify(updatedConfig));
    }
  };
  ...
  const handleToggleSize = (widgetId: string) => {
    setLayout(prevLayout => {
      ...
      setConfig(prevConfig => {
        ...
        localStorage.setItem('enterpriseDashboardConfig_mecanicien', JSON.stringify(newConfig));
        return newConfig;
      });
  ...
  const handleRemoveWidget = (widgetId: string) => {
    ...
    if (currentLayoutItem) {
      ...
      const existingBackup = localStorage.getItem('enterpriseDashboardConfig_mecanicien_backup');
      ...
      localStorage.setItem('enterpriseDashboardConfig_mecanicien_backup', JSON.stringify(backupConfig));
  ...
  const handleAddWidget = (widgetId: string) => {
    ...
    const widgetToAdd = MecanicienWidgets.widgets.find(w => w.id === widgetId);
    if (!widgetToAdd) {
    ...
    const savedLayout = localStorage.getItem('enterpriseDashboardConfig_mecanicien_backup');
  ...
  const handleRestoreAllWidgets = () => {
    ...
    const allWidgets = MecanicienWidgets.widgets;
  ...
  const handleSaveDashboard = () => {
    ...
    localStorage.setItem('enterpriseDashboardConfig_mecanicien', JSON.stringify(currentConfig));
        ...
        {showAddWidgetModal && (
          <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md">
              <h2 className="text-lg font-bold mb-4">Ajouter un widget mécanicien</h2>
              <div className="mb-2 text-xs text-gray-500">
                Widgets disponibles: {MecanicienWidgets.widgets.length} | 
                Widgets filtrés: {MecanicienWidgets.widgets.filter(widget => WIDGETS_MECANICIEN_IDS.includes(widget.id)).length}
              </div>
              <ul>
                {MecanicienWidgets.widgets
                  .filter(widget => WIDGETS_MECANICIEN_IDS.includes(widget.id))
                  .map(widget => {
