...
const EnterpriseDashboardCourtierDisplay: React.FC = () => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('enterpriseDashboardConfig_courtier');
    let parsed: any = null;
    if (saved) {
      try {
        parsed = JSON.parse(saved);
      } catch {
        localStorage.removeItem('enterpriseDashboardConfig_courtier');
      }
    }
    ...
    if (!parsed || !parsed.widgets || !Array.isArray(parsed.widgets)) {
      ...
      const allWidgets = CourtierWidgets.widgets;
    ...
    if (shouldUpdate) {
      ...
      localStorage.setItem('enterpriseDashboardConfig_courtier', JSON.stringify(parsed));
    }
  ...
  useEffect(() => {
    if (config) {
      ...
      const allWidgetIds = CourtierWidgets.widgets.map(w => w.id);
  ...
  const onLayoutChange = (newLayout: any[], allLayouts?: any) => {
    ...
    if (config) {
      ...
      localStorage.setItem('enterpriseDashboardConfig_courtier', JSON.stringify(updatedConfig));
    }
  };
  ...
  const handleToggleSize = (widgetId: string) => {
    setLayout(prevLayout => {
      ...
      setConfig(prevConfig => {
        ...
        localStorage.setItem('enterpriseDashboardConfig_courtier', JSON.stringify(newConfig));
        return newConfig;
      });
  ...
  const handleRemoveWidget = (widgetId: string) => {
    ...
    if (currentLayoutItem) {
      ...
      const existingBackup = localStorage.getItem('enterpriseDashboardConfig_courtier_backup');
      let backupConfig = { layout: { lg: [] } };
      if (existingBackup) {
        try {
          backupConfig = JSON.parse(existingBackup);
        } catch {
          localStorage.removeItem('enterpriseDashboardConfig_courtier_backup');
        }
      }
      ...
      localStorage.setItem('enterpriseDashboardConfig_courtier_backup', JSON.stringify(backupConfig));
  ...
  const handleAddWidget = (widgetId: string) => {
    ...
    const widgetToAdd = CourtierWidgets.widgets.find(w => w.id === widgetId);
    if (!widgetToAdd) {
    ...
    const savedLayout = localStorage.getItem('enterpriseDashboardConfig_courtier_backup');
  ...
  const handleSaveDashboard = () => {
    ...
    logger.info('✅ Tableau de bord Enterprise Courtier sauvegardé et configuré');
        ...
        {showAddWidgetModal && (
          <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md">
              <h2 className="text-lg font-bold mb-4">Ajouter un widget courtier</h2>
              <div className="mb-2 text-xs text-gray-500">
                Widgets disponibles: {CourtierWidgets.widgets.length} | 
                Widgets filtrés: {CourtierWidgets.widgets.filter(widget => WIDGETS_COURTIER_IDS.includes(widget.id)).length}
              </div>
              <ul>
                {CourtierWidgets.widgets
                  .filter(widget => WIDGETS_COURTIER_IDS.includes(widget.id))
                  .map(widget => {
