...
const EnterpriseDashboardTransitaireDisplay: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (shouldUpdate) {
      ...
      localStorage.setItem('enterpriseDashboardConfig_transitaire', JSON.stringify(parsed));
    }
  ...
  const onLayoutChange = (newLayout: any[], allLayouts?: any) => {
    ...
    if (config) {
      ...
      localStorage.setItem('enterpriseDashboardConfig_transitaire', JSON.stringify(updatedConfig));
    }
  };
  ...
  const handleToggleSize = (widgetId: string) => {
    setLayout(prevLayout => {
      ...
      setConfig(prevConfig => {
        ...
        localStorage.setItem('enterpriseDashboardConfig_transitaire', JSON.stringify(newConfig));
        return newConfig;
      });
  ...
  const handleAddWidget = (widgetId: string) => {
    ...
    const widgetToAdd = TransitaireWidgets.widgets.find(w => w.id === widgetId);
    if (!widgetToAdd) {
    ...
    const savedLayout = localStorage.getItem('enterpriseDashboardConfig_transitaire_backup');
  ...
  const handleRestoreAllWidgets = () => {
    ...
    const allWidgets = TransitaireWidgets.widgets;
  ...
  const handleSaveDashboard = () => {
    ...
    localStorage.setItem('enterpriseDashboardConfig_transitaire', JSON.stringify(currentConfig));
        ...
        {showAddWidgetModal && (
          <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md">
              <h2 className="text-lg font-bold mb-4">Ajouter un widget transitaire</h2>
              <div className="mb-2 text-xs text-gray-500">
                Widgets disponibles: {TransitaireWidgets.widgets.length} | 
                Widgets filtrés: {TransitaireWidgets.widgets.filter(widget => WIDGETS_TRANSITAIRE_IDS.includes(widget.id)).length}
              </div>
              <ul>
                {TransitaireWidgets.widgets
                  .filter(widget => WIDGETS_TRANSITAIRE_IDS.includes(widget.id))
                  .map(widget => {
