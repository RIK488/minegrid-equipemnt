...
-- Table profiles avec les bonnes colonnes (firstname, lastname en minuscules)
CREATE TABLE profiles (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  email TEXT UNIQUE NOT NULL,
  firstname TEXT,  -- En minuscules comme attendu par le code
  lastname TEXT,   -- En minuscules comme attendu par le code
  avatar_url TEXT,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- Table messages avec les bonnes relations
...
-- Table offers avec relations vers machines
CREATE TABLE offers (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- Table machine_views avec la bonne colonne viewed_at
...
CREATE POLICY "Machine views are viewable by machine owners" ON machine_views
  FOR SELECT USING (
    EXISTS (
      ...
      AND machines.seller_id::text = auth.uid()::text
    )
  );
...
-- Créer la fonction exchange-rates
...
DECLARE
  result JSON;
BEGIN
  -- Retourner des taux de change fixes pour l'instant
  result := '{
    "EUR": 1.0,
    "USD": 1.08,
    "MAD": 10.8,
...
-- Insérer quelques vues de test pour les machines existantes
